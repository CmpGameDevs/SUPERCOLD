...
vec3 getBoneColor(int boneIndex) {
    // Use golden ratio to generate well-distributed colors
    float golden_ratio = 0.618033988749895;
    float hue = fract(float(boneIndex) * golden_ratio);
    
    // Create HSV color
    vec3 hsv = vec3(hue, 0.8, 0.95); // hue, saturation, value
...
void main() {
    vec3 boneVisualization = vec3(0.0);

    for(int i = 0; i < 4; i++) {
        if(boneWeightsOut[i] > 0.0) {
            vec3 boneColor = getBoneColor(boneIndicesOut[i]);
            boneVisualization += boneColor * boneWeightsOut[i];
        }
    }

    float totalWeight = boneWeightsOut.x + boneWeightsOut.y + boneWeightsOut.z + boneWeightsOut.w;
    vec2 coord = gl_FragCoord.xy / 20.0; // Adjust scale as needed
    float pattern = mod(coord.x + coord.y, 2.0) < 1.0 ? 0.8 : 1.0;
    vec3 finalColor = boneVisualization * pattern;
    float multipleBonesIntensity = float(boneWeightsOut.y > 0.0) * 0.2;
    finalColor += vec3(multipleBonesIntensity);
    ...
    FragColor = vec4(mix(color, finalColor, 0.7), 1.0);
