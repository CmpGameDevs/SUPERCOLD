...
our::Crosshair::Crosshair() :
     mesh(nullptr), material(nullptr), color(1.0f, 1.0f, 1.0f, 1.0f) {}
...
void our::Crosshair::parseConfig(const nlohmann::json &config) {
    ...
    if (config.contains("color") && config["color"].is_array() && config["color"].size() == 4) {
        color = glm::vec4(
            config["color"][0].get<float>(),
            config["color"][1].get<float>(),
            config["color"][2].get<float>(),
            config["color"][3].get<float>()
        );
    } else {
...
void our::Crosshair::initialize(const nlohmann::json &config) {
    ...
    our::Color vertexColor = {static_cast<uint8_t>(color.r * 255), static_cast<uint8_t>(color.g * 255), static_cast<uint8_t>(color.b * 255), static_cast<uint8_t>(color.a * 255)};
